...
> 💡 SSEは「**サーバー → ブラウザ**」の**一方通行**です。ブラウザから「やっぱり止めて！」と**逆向きに割り込む**話は、双方向のしくみ（WebSocket）が得意で、**次の第12章**で扱います。
...
次の第12章では、**行き来が自由な双方向通信（WebSocket）** を学びます。「入力中…」の表示や、**返事の途中で止める**——そんなリッチな体験へ進みましょう。

[→ 第12章 双方向通信（WebSocket編）— 入力中・割り込み（止める）へ](12-websocket.md)
