...
## 🔌 もう一歩：MCP — ツールを「プラグイン」にする標準（🔧 応用）

ここまでは、道具（`tools`）を**自分のアプリの中に1つずつ手書き**しました。便利な道具が増えるほど、似た実装をあちこちで書くことになります。

そこで出てきたのが **MCP（Model Context Protocol／エムシーピー）**。ひとことで言うと、**「ツールを、アプリの外の“プラグイン”として共通の形でつなぐ」標準**です。

- 道具を提供する側＝**MCPサーバー**（例：ファイル／GitHub／データベース／天気…）。tools の **説明書も実体もサーバーが持つ**。
- 使う側＝**MCPクライアント**（あなたのアプリ）。決まった規格でつなぐだけで、そのサーバーの道具が**そのまま使える**。

つまり MCP は **function calling の“応用・上の層”**です。LLMの裏側は相変わらず「`tool_call` で道具を呼ぶ」——ただ、その道具を**毎回自作するのではなく、共通規格のサーバーから“挿して使う”**ようにしたもの。**USB-C** のように、いろんなAIアプリといろんな道具を**同じ差し込み口**でつなぐイメージです。

> 💡 本章の `getWeather`（説明書＋実体＋`tool_call` で呼ぶ）が分かっていれば、MCP は **「その道具を外に出して共通規格で配る／使う」** だけ。**検証・最小権限・確認**などの🛡守りも、今度は **“外部のMCPサーバーを信用しすぎない”** 形でそのまま効きます。
...
- **MCP（Model Context Protocol）**：ツールを**アプリの外のサーバー**から共通規格で“挿して使う”しくみ。function calling の応用・上の層（裏では同じ `tool_call`）
